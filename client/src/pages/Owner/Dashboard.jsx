...
const Dashboard = () => {
    const {axios, isOwner, currency, dashboardRefreshTrigger} = useAppContext();
    ...
    const fetchDashBoardData = async () => {
        try{
            ...
            if(response.data.success)
            {
                ...
                console.log('Total Cars : ', response.data.dashboardData.totalCars);
                console.log('Total Bookings : ', response.data.dashboardData.totalBookings);
                console.log('Recent Bookings : ', response.data.dashboardData.recentBookings);
        ...
        }catch(error)
        {
            ...
            const errorMessage = error.response?.data?.message || error.message || "Unauthorized access";
    ...
    }, [isOwner, dashboardRefreshTrigger, location.state?.refresh]);
    ...
    return (
        <div className="px-4 pt-10 md:px-10 flex-1">
            ...
            <div className="flex justify-end mb-4">
                <button onClick={fetchDashBoardData} className="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary/90 transition-colors text-sm cursor-pointer">Refresh Data</button>
            </div>
