...
const checkAvailabiltiy = async (car, pickupDate, returnDate) => {
...
export const checkAvailabilityOfCar = async (req, res) => {
    try{
        ...
        // fetch all available cars for the given locations
        const cars = await Car.find({location, isAvailable : true});

        // check car availability for the given date range using promise
        const availableCarPromise = cars.map(async (car) => {
            const isAvailable = await checkAvailabiltiy(car._id, pickupDate, returnDate);
            return {...car._doc, isAvailable : isAvailable}
        })

        let availableCars = await Promise.all(availableCarPromise);
        avaialbelCars = availableCars.filter(car => car.isAvailable === true);
...
// write an API to create car booking
